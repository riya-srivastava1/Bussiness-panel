...
use App\HoldTimeSlot;
use App\Http\Controllers\NotificationTraitBusiness;
use App\Http\Controllers\TraitClass\SMSTrait;
use App\ManageMktCity;
use App\Models\UserBooking;
use App\VendorStylist;
use App\VendorConfirmationNotification;
...
class BusinessController extends Controller
{
    ...
    //     $payload = ['push-type' => 'confirmation', 'status' => 'accept'];
    ...
    //     return view('business.dashboard.my-account', compact(['category', 'serviceType', 'cities']));
    // }
    ...
    //     return view('business.dashboard.manage-date-time', compact(['disabledDate']));
    // }
    ...
    //     return view('business.dashboard.branch', compact(['branchs']));
    // }
    ...
    //     $category = Category::whereIn('status', [1, '1'])->get();
    //     $cities = ManageMktCity::all();
    //     return view('business.dashboard.add-branch', compact(['branchs', 'category', 'cities']));
    // }
    ...
    public function userBooking(Request $request)
    {
        ...
        if ($request->type == "cancelled") {
            ...
            $pendingBooking = UserBooking::where('vendor_id', $vendor_id)
                ...
                ->orderByDesc('updated_at')
                ->count();
            return view('business.dashboard.user-booking-cancelled', compact(['canclledBooking', 'completeBooking', 'pendingBooking', 'vendor_profile', 'notifyBranchs']));
            ...
            $pendingBooking = UserBooking::where('vendor_id', $vendor_id)
                ...
                ->orderByDesc('updated_at')
                ->paginate(15);
            return view('business.dashboard.user-booking-pending', compact(['canclledBooking', 'completeBooking', 'pendingBooking', 'vendor_profile', 'notifyBranchs']));
        } else {
            $canclledBooking = UserBooking::where('vendor_id', $vendor_id)
                ->where('booking_status', true)
                ->where('service_status', '!=', true)
            ...
            $pendingBooking = UserBooking::where('vendor_id', $vendor_id)
                ...
                ->orderByDesc('updated_at')
                ->count();
            return view('business.dashboard.user-booking-complete', compact(['canclledBooking', 'completeBooking', 'pendingBooking', 'vendor_profile', 'notifyBranchs']));
        }
    }
    ...
    public function userBookingDetails(Request $request)
    {
        ...
        if ($booking && $booking->count()) {
            $type = $request->type;
            return view('business.dashboard.user-booking-details', compact(['booking', 'type']));
        }
        return back();
    }
