  ...
  <div class="row gutters">
    ...
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
      <div class="card bg-white">
        <div class="card-header">Manage Branches <span class="float-right"><a href="{{ route('vendor.add.branch') }}" class="btn btn-success">Add a Branch</a></span></div>
          ...
          <table class="table table-bordered text-center" style="width: 100% !important">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <form action="{{ route('vendor.branch.edit.permission') }}" id="edit_permission_form" method="post">
                    @csrf
                    <input type="hidden" name="vendor_id" value="{{ $item->vendor_id }}">
                    @php $edit_permission = $item->vendor_profile->edit_permission ?? ''; @endphp
                    <select class="form-control" name="edit_permission" id="edit_permission_select" required>
                      <option value="">Select Editing permisssion</option>
                      <option {{ $edit_permission=='Read' ? 'selected' : '' }} value="Read">Read</option>
                      <option {{ $edit_permission=='Write' ? 'selected' : '' }} value="Write">Write</option>
                    </select>
                  </form>
                </td>
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyC3tBEtsH0JPA8Hh-lbBphyfgZM5KY0Hko&libraries=places&callback=initMap" async defer></script>
