...
@section('content')
    <div id="content" class="app-content">
        <div class="row">
            <!-- BEGIN .main-content -->
            <div class="col-md-4">
                <a href="?type=complete">
                    <h5 class=" badge-pill badge-success py-2">Completed Booking &mdash;
                        {{ $completeBooking ? $completeBooking->total() : 0 }} </h5>
                </a>
            </div>
                    ...
                    <div class="card-body">
                        @if ($notifyBranchs && $notifyBranchs->count())
                            <div class="bg-warning my-3 px-3 pt-3 pb-2 text-center rounded" style="max-width: 350px">
                                <select class="form-control selectpicker" data-live-search="true" id="notifyBranch">
                                    ...
                                    @endforeach
                                    <option id="{{ Auth::guard('business')->user()->vendor_id ?? '' }}"
                        ...
                        @if ($completeBooking)
                            <div class="form-row">
                                ...
                                @foreach ($completeBooking as $item)
                                    @if ($item->services)
                                        @php
                                    ...
                                    <div class="booking-list-completed">
                                        <a
                                            href="{{ route('business.dashboard.user.booking.details', ['order_id' => $item->order_id, 'type' => 'Completed']) }}">
                            ...
                            <div class="d-flex justify-content-center">
                                {{ $completeBooking->links('pagination::bootstrap-4') }}
                            </div>
                        @endif
